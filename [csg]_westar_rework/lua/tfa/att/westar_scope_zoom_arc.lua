...
ATTACHMENT.Name = "Variable Westar Scope"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = {
TFA.Attachments.Colors["="], "2.5-6x adjustable zoom",
...
function ATTACHMENT:Attach( wep )
	...
	if CLIENT or game.SinglePlayer() then
		hook.Add( "PlayerBindPress", "VariableZoom", function( ply, bind, pressed )
			if ply:IsValid() and ply:GetActiveWeapon().IsScopeActive and pressed and ply:GetActiveWeapon():GetClass() == self.Weapon:GetClass() and string.find( bind, "+zoom" ) and ply:GetActiveWeapon().CLIronSightsProgress > 0.5 then
				print(self.Weapon:GetClass())
				surface.PlaySound( "buttons/blip1.wav" )
				if fov == 7 then fov = 3 else fov = 7 end
				if ( string.find( bind, "+zoom" ) ) then return true end
			end
		end )
	end
	--]]
	wep.IronSightsSensitivity = 0.3
