...
SWEP.PrintName						= "Westar-M5 ARC"
SWEP.Type							= "Modified ARC Rifle"
SWEP.DrawAmmo						= true
...
SWEP.Attachments = {
	...
	[2] = { offset = { 0, 0 }, atts = { "westar_scope_zoom_arc", "westar_scope_4x" }, order = 2 },
	[3] = { offset = { 0, 0 }, atts = { "westar_ammo_shotgun", "westar_ammo_sniper", "arc_ammo_stun" }, order = 3 }, -- EDIT: Added new attachments and implemented scrapped content
	[4] = { offset = { 0, 0 }, atts = { "dcs_laser", "dcs_ammo" }, order = 4 }, -- EDIT: Added new attachments and implemented scrapped content
}
...
function SWEP:Think()
	...
	if CLIENT or game.SinglePlayer() then
		hook.Add( "PlayerBindPress", "VariableZoom", function( ply, bind, pressed )
			if pressed and ply:IsValid() and ply:GetActiveWeapon() == self and ply:GetActiveWeapon():IsAttached( "westar_scope_zoom_arc" ) and string.find( bind, "+zoom" ) and self.CLIronSightsProgress > 0.5 then
				...
				self:Detach( "westar_scope_zoom_arc" )
				self:Attach( "westar_scope_zoom_arc" )
			return true end
		end )
	end
end